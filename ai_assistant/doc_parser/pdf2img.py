...
def pdfDoc2img(pdfPath: str) -> List[Image.Image]:
    reader = PdfReader(pdfPath)
    num_pages = len(reader.pages)
    logger.info(f"Total pages in PDF: {num_pages}")

    all_images = []
    for pg_num in tqdm(range(1, num_pages + 1), desc="Processing PDF Pages"):
        # Load image from PDF
        image = render_pdf_to_image(pdfPath, pg_num, target_longest_image_dim=1024)
